# JavaScript Interview Questions — sudheerj/javascript-interview-questions
> Source: https://github.com/sudheerj/javascript-interview-questions/blob/master/README.md

---

## Main Interview Questions (1–478)
...
130. How do you make the first letter of the string in uppercase?
...
137. Is the `!--` notation a special operator?
...
148. What are the recommendations to create new objects?
...
191. How do you determine if two values are the same using Object?
...
211. How do you encode a URL?
212. How do you decode a URL?
213. How do you print the contents of a web page?
...
236. How does synchronous iteration work?
...
260. What happens if I pass a string type for `getPrototypeOf` method?
261. How do you set the prototype of one object to another?
262. How do you check whether an object can be extended or not?
263. How do you prevent an object from being extended?
...
351. Does JavaScript use mixins?
352. Give a mixin example using Object composition.
353. What are the benefits of mixins?
...
377. How do you create a copy to clipboard button?
378. What is the shortcut to get a timestamp?
379. How do you flatten multi dimensional arrays?
380. What is the easiest multi condition checking?
381. How do you capture the browser back button?
...
415. How do you check if an object is a promise or not?
...
425. Is it possible to use expressions in switch cases?
426. What is the easiest way to ignore promise errors?
427. How do you style the console output using CSS?
...
433. How do you reverse an array without modifying the original array?
434. How do you create a custom HTML element?
...
458. What are the benefits of higher order functions?
...
## Coding Exercise Questions (Output-based)

1. What is the output of below code? (`new Vehicle("Honda", "white", "2010", "UK")`)
2. What is the output of below code? (`let x = (y = 0); x++; y++;`)
3. What is the output of below code? (`main()` function with `setTimeout` and two `console.log`)
...
7. What is the output of below code? (`delete myChars[0]` on an array)
...
24. What is the output of below code? (`async function func() { return 10; }`)
25. What is the output of below code? (`async function func() { await 10; }`)
26. What is the output of below code? (`processArray` using `forEach` with `await`)
27. What is the output of below code? (`process` using `async forEach` with `console.log("Process completed!")`)
28. What is the output of below code? (`new Set().add("+0").add("-0").add(NaN).add(undefined).add(NaN)`)
29. What is the output of below code? (`Symbol("one") === Symbol("one")` and `Symbol.for("two") === Symbol.for("two")`)
30. What is the output of below code? (`const sym1 = new Symbol("one")`)
31. What is the output of below code? (`!typeof myNumber === "string"`)
32. What is the output of below code? (`JSON.stringify` with `undefined`, functions, and Symbols)
33. What is the output of below code? (`new.target.name` in class inheritance)
34. What is the output of below code? (`const [x, ...y, z] = [1, 2, 3, 4]`)
35. What is the output of below code? (`const { a: x = 10, b: y = 20 } = { a: 30 }`)
36. What is the output of below code? (`function area({ length = 10, width = 20 })` called without arguments)
37. What is the output of below code? (Array + object nested destructuring `[, , { name }] = props`)
38. What is the output of below code? (`checkType(num = 1)` called with `undefined`, `""`, `null`)
39. What is the output of below code? (`function add(item, items = [])` called twice)
40. What is the output of below code? (`greet("Hello", "John")` with `message = greeting + " " + name` default)
41. What is the output of below code? (`function outer(f = inner())` where `inner` is declared inside)
42. What is the output of below code? (`function myFun(x, y, ...manyMoreArgs)` called with and without extra args)
43. What is the output of below code? (`const obj = { key: "value" }; const array = [...obj]`)
44. What is the output of below code? (`var myGenObj = new myGenFunc()`)
45. What is the output of below code? (generator with `yield 1; return 2; yield 3`)
46. What is the output of below code? (generator with `for...of` loop and `break`, used twice)
47. What is the output of below code? (`const num = 0o38`)
48. What is the output of below code? (`new Square(10)` used before class declaration)
49. What is the output of below code? (`walk` and `run` called without object context)
50. What is the output of below code? (`Car extends Vehicle` where `Car.start()` calls `super.start()`)
51. What is the output of below code? (`const USER = { age: 30 }; USER.age = 25`)
52. What is the output of below code? (`console.log("🙂" === "🙂")`)
53. What is the output of below code? (`console.log(typeof typeof typeof true)`)
54. What is the output of below code? (`let zero = new Number(0); if (zero) {...}`)
55. What is the output of below code in non-strict mode? (`msg.name = "John"` on a string primitive)
56. What is the output of below code? (closure with `let count = 10` and inner `let count = 11`)
57. What is the output of below code? (`console.log(true && 'hi')`, `true && 'hi' && 1`, `true && '' && 0`)
58. What is the output of below code? (`let arr = [1,2,3]; let str = "1,2,3"; console.log(arr == str)`)
59. What is the output of below code? (`getMessage()` called before arrow function assignment)
60. What is the output of below code? (`quickPromise.then(...)` vs `console.log("program finished")`)
61. What is the output of below code? (`console.log("First line")` followed by chained array and `console.log("Third line")`)
62. Write a function that returns a random HEX color.
63. What is the output of below code? (`var of = ["of"]; for (var of of of) {...}`)
64. What is the output of below code? (`[11, 25, 31, 23, 33, 18, 200].sort()`)
65. What is the output order of below code? (`setTimeout`, `Promise.resolve`, `console.log("3")`)
66. What is the output of below code? (`console.log(name)` and IIFE `(function message() {})`)
67. What is the output of below code? (`message()` with two function declarations of same name)
68. What is the output of below code? (var hoisting in `changeCurrentCity` function)
69. What is the output of below code in order? (`second()`, `first()`, global `message`)
70. What is the output of below code? (`var expressionOne = function functionOne() {}; functionOne()`)
71. What is the output of below code? (`user.eat()` with inner `eatFruit` regular function and `this`)
72. What is the output of below code? (string immutability — `message[0] = "J"` and string concatenation)
73. What is the output of below code? (`user1 === user2` with two identical objects)
74. What is the output of below code? (`setTimeout` capturing `const message` declared after it)
75. What is the output of below code? (`const a = new Number(10); const b = 10; console.log(a === b)`)
76. What is the type of below function? (`function add(a, b) { console.log(...); return a + b; }`)
77. What is the output of below code? (`Promise.all` with one resolve and one reject)
78. What is the output of below code? (`try { setTimeout(() => { throw new Error(...) }) } catch(err) {}`)
79. What is the output of below code? (`let a = 10; if (true) { let a = 20; ... }`)
80. What is the output of below code? (`arr.length = 0` on a non-empty array)
81. How do you verify two strings are anagrams or not?
82. What is the output of below code? (`printHello()` and `printMessage()` with nested function hoisting)
83. What is the time taken to execute below timeout callback? (`setTimeout` with 5 seconds delay inside a 10-second while loop)
84. What is the output of below code? (`["wöchentlich", "Woche", "wäre", "Wann"].sort()`)
85. What is the output of below code? (`func(undefined)` vs `func()` with default parameter and `arguments.length`)
86. What is the output of below code? (`numbers.reduce` counting odd values followed by an IIFE using XOR bit shifting)
